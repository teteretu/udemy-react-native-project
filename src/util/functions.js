const createBoard = (rows, columns) => {
  // create array with index number e each item of array
  return Array(rows)
    .fill(0)
    .map((_, row) => {
...
const safeNeighborhood = (board, row, column) => {
  // diferente de minado porque se estiver algum true em safes, o .reduce retornará false
  // [false, true, false] utilizando o .reduce() retorna false
  // [false, false, false] utilizando o .reduce() retorna true
...
const invertFlag = (board, row, column) => {
  const field = board[row][column];
  if (!field.opened && !field.exploded && field.nearMines <= 0)
